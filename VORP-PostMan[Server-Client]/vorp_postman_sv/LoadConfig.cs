    ...
    class LoadConfig : BaseScript
    {
        ...
        private void LoadConfigAndLang()
        {
            if (File.Exists($"{resourcePath}/Config.json"))
            {
                ...
                if (File.Exists($"{resourcePath}/locale/{Config["defaultlang"]}.json"))
                {
                    string langstring = File.ReadAllText($"{resourcePath}/locale/{Config["defaultlang"]}.json", Encoding.UTF8);
